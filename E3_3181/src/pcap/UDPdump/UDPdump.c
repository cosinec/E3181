...
pcap_if_t* selectDev(int DevsCount) {
	int choice;
	printf("Please enter the interface number(1-%d):", DevsCount);
	scanf("%d", &choice);

	if (choice <= 0 || choice > DevsCount) {
		printf("Incorrect input. Please confirm your input.\n", DevsCount, choice);
...
pcap_t* getHandle(pcap_if_t* dev) {
	...
	if ((handle = pcap_open_live(dev->name, 65536, 1, 1000, error)) == NULL) {
		printf("Failed to open the interface adapter %s\n", dev->name);
	...
	if (pcap_datalink(handle) != DLT_EN10MB) {
		printf("Please use in Ethernet environment!\n");
		pcap_freealldevs(all_devs);
		exit(-1);
	}
	return handle;
